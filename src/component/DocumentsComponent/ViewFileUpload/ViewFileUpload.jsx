    ...
    <div className="w-[100%] h-[100%]">
      ...
      <ul
        className={`w-[100%]  transition-all duration-500  ${
          showMore
            ? "h-[140px] md:h-[160px] lg:h-[220px] grid-cols-2 grid sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-x-[2%]"
            : "h-[140px] md:h-[160px] lg:h-[220px] flex gap-x-[2%] items-center justify-start overflow-x-scroll flex-nowrap"
        }`}
      >
        {documents.map((doc, index) => (
          <li
            ...
            key={index}
            className="min-w-[20%] w-[100%]  h-[100%] relative overflow-hidden flex items-center justify-center flex-col  cursor-pointer hover:shadow-2xl transition-all duration-300"
          >
            <FcFolder className="text-[3rem] md:text-[4rem] lg:text-[5rem]" />
            <h5 className="w-[75%] text-[1rem] text-center  truncate  ">
              {doc.name}
            </h5>
            <h6 className="text-[0.9rem] text-black-2">{doc.size} (Kb)</h6>
            <MdDownload
              className="absolute top-[1%] right-[2%] text-[1.3rem] md:text-[1.4rem] lg:text-[1.6rem]  cursor-pointer text-black-2"
              onClick={() => {
